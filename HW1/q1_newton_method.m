% Implement Newtons method for optimizing l(theta) and apply it to fit a logistic regression model to the data.
...
function theta = newtonsMethod(X,y)
	...
	for k = 1:max_iters
		for i = 1:m
			...
			if abs(new_theta - theta) < exp(-6) return;
			end
			theta = new_theta;
		end
	end
...
function plotTrainingData(X,y,theta)
	...
	% Decision boundary is values of x1, x2 such that hx = 0.5
	a = linspace(0, 8);
